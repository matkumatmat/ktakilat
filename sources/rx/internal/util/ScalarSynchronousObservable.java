...
public final class ScalarSynchronousObservable<T> extends Observable<T> {
    static final boolean STRONG_MODE = Boolean.valueOf(System.getProperty("rx.just.strong-mode", BooleanUtils.FALSE)).booleanValue();
    final T t;
    ...
    public static final class WeakSingleProducer<T> implements Producer {
        ...
        public void request(long j) {
            if (!this.once) {
                int i = (j > 0 ? 1 : (j == 0 ? 0 : -1));
                if (i < 0) {
                    throw new IllegalStateException(e.j(j, "n >= required but it was "));
                } else if (i != 0) {
                    this.once = true;
                    Subscriber<? super T> subscriber = this.actual;
                    if (!subscriber.isUnsubscribed()) {
    ...
    public static <T> Producer createProducer(Subscriber<? super T> subscriber, T t2) {
        if (STRONG_MODE) {
