...
public final class HttpHeaders {
    ...
    public static final List<Challenge> parseChallenges(@NotNull Headers headers, @NotNull String str) {
        ...
        for (int i = 0; i < size; i++) {
            if (StringsKt.q(str, headers.name(i), true)) {
                try {
    ...
    public static final boolean promisesBody(@NotNull Response response) {
        ...
        if (Intrinsics.a(response.request().method(), FirebasePerformance.HttpMethod.HEAD)) {
            return false;
        }
        int code = response.code();
        if (((code >= 100 && code < 200) || code == 204 || code == 304) && Util.headersContentLength(response) == -1 && !"chunked".equalsIgnoreCase(Response.header$default(response, com.google.common.net.HttpHeaders.TRANSFER_ENCODING, (String) null, 2, (Object) null))) {
            return false;
        }
        return true;
    }
            ...
            while (true) {
                ...
                if (readToken != null) {
                    ...
                    if (skipCommasAndWhitespace || (!skipCommasAndWhitespace2 && !buffer.exhausted())) {
                        ...
                        int skipAll2 = Util.skipAll(buffer, (byte) 61) + skipAll;
                        while (true) {
                            if (readToken == null) {
                                readToken = readToken(buffer);
                    ...
                    list.add(new Challenge(str2, (Map<String, String>) MapsKt.b()));
                    return;
                } else {
                    return;
                }
            }
            StringBuilder v = ba.v(readToken);
            v.append(StringsKt.C("=", skipAll));
            Map singletonMap = Collections.singletonMap((Object) null, v.toString());
    ...
    private static final boolean startsWith(Buffer buffer, byte b) {
        if (buffer.exhausted() || buffer.getByte(0) != b) {
            return false;
        }
        return true;
    }
