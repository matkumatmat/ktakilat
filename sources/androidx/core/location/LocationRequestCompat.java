...
public final class LocationRequestCompat {
    ...
    public String toString() {
        StringBuilder v = ba.v("Request[");
        if (this.mIntervalMillis != PASSIVE_INTERVAL) {
            v.append("@");
            TimeUtils.formatDuration(this.mIntervalMillis, v);
            int i = this.mQuality;
            if (i == 100) {
                v.append(" HIGH_ACCURACY");
            } else if (i == 102) {
                v.append(" BALANCED");
            } else if (i == 104) {
                v.append(" LOW_POWER");
        ...
        if (this.mDurationMillis != PASSIVE_INTERVAL) {
            v.append(", duration=");
            TimeUtils.formatDuration(this.mDurationMillis, v);
        }
        if (this.mMaxUpdates != Integer.MAX_VALUE) {
            v.append(", maxUpdates=");
            v.append(this.mMaxUpdates);
        }
        long j = this.mMinUpdateIntervalMillis;
        if (j != -1 && j < this.mIntervalMillis) {
            v.append(", minUpdateInterval=");
            TimeUtils.formatDuration(this.mMinUpdateIntervalMillis, v);
        }
        if (((double) this.mMinUpdateDistanceMeters) > 0.0d) {
            v.append(", minUpdateDistance=");
            v.append(this.mMinUpdateDistanceMeters);
        }
        if (this.mMaxUpdateDelayMillis / 2 > this.mIntervalMillis) {
            v.append(", maxUpdateDelay=");
            TimeUtils.formatDuration(this.mMaxUpdateDelayMillis, v);
        }
        v.append(']');
        return v.toString();
    }
    ...
    public LocationRequest toLocationRequest(@NonNull String str) {
        ...
        return v0.f(Api19Impl.toLocationRequest(this, str));
    }
