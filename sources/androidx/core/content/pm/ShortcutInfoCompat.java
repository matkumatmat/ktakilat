...
public class ShortcutInfoCompat {
    ...
    public static class Builder {
                ...
                if (this.mCapabilityBindingParams != null) {
                    ...
                    for (String next : this.mCapabilityBindingParams.keySet()) {
                        ...
                        for (String str : map.keySet()) {
                            ...
                            String m = e.m(next, RemoteSettings.FORWARD_SLASH_STRING, str);
                            if (list == null) {
                                strArr = new String[0];
                            } else {
        ...
        public Builder(@NonNull Context context, @NonNull ShortcutInfo shortcutInfo) {
            ...
            Intent[] x = shortcutInfo.getIntents();
            shortcutInfoCompat.mIntents = (Intent[]) Arrays.copyOf(x, x.length);
    ...
    public static List<ShortcutInfoCompat> fromShortcuts(@NonNull Context context, @NonNull List<ShortcutInfo> list) {
        ...
        for (ShortcutInfo h : list) {
            arrayList.add(new Builder(context, g6.h(h)).build());
        }
        return arrayList;
    }
    ...
    public ShortcutInfo toShortcutInfo() {
        g6.o();
        ShortcutInfo.Builder f = g6.d(this.mContext, this.mId).setShortLabel(this.mLabel).setIntents(this.mIntents);
        IconCompat iconCompat = this.mIcon;
        if (iconCompat != null) {
            f.setIcon(iconCompat.toIcon(this.mContext));
        }
        if (!TextUtils.isEmpty(this.mLongLabel)) {
            f.setLongLabel(this.mLongLabel);
        }
        if (!TextUtils.isEmpty(this.mDisabledMessage)) {
            f.setDisabledMessage(this.mDisabledMessage);
        }
        ComponentName componentName = this.mActivity;
        if (componentName != null) {
            f.setActivity(componentName);
        }
        Set<String> set = this.mCategories;
        if (set != null) {
            f.setCategories(set);
        }
        f.setRank(this.mRank);
        PersistableBundle persistableBundle = this.mExtras;
        if (persistableBundle != null) {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            ...
            if (personArr != null && personArr.length > 0) {
                ...
                f.setPersons(personArr2);
            }
            LocusIdCompat locusIdCompat = this.mLocusId;
            if (locusIdCompat != null) {
                f.setLocusId(locusIdCompat.toLocusId());
            }
            f.setLongLived(this.mIsLongLived);
        } else {
            f.setExtras(buildLegacyExtrasBundle());
        }
        if (Build.VERSION.SDK_INT >= 33) {
            Api33Impl.setExcludedFromSurfaces(f, this.mExcludedSurfaces);
        }
        return f.build();
    }
