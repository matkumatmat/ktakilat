...
public final class PackageInfoCompat {
    ...
    public static List<Signature> getSignatures(@NonNull PackageManager packageManager, @NonNull String str) throws PackageManager.NameNotFoundException {
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            SigningInfo h = packageManager.getPackageInfo(str, 134217728).signingInfo;
            if (Api28Impl.hasMultipleSigners(h)) {
                signatureArr = Api28Impl.getApkContentsSigners(h);
            } else {
                signatureArr = Api28Impl.getSigningCertificateHistory(h);
            }
        } else {
        ...
        for (byte[] next : keySet) {
            if (next != null) {
                ...
                if (num != null) {
                    ...
                    throw new IllegalArgumentException(e.B("Type must be specified for cert when verifying ", str));
                }
            } else {
                throw new IllegalArgumentException(e.B("Cert byte array cannot be null when verifying ", str));
            }
        }
