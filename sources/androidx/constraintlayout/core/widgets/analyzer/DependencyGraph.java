...
public class DependencyGraph {
    ...
    private String generateDisplayNode(DependencyNode dependencyNode, boolean z, String str) {
        ...
        for (DependencyNode name : dependencyNode.targets) {
            StringBuilder s = e.s(StringUtils.LF + dependencyNode.name(), " -> ");
            s.append(name.name());
            String sb2 = s.toString();
            if (dependencyNode.margin > 0 || z || (dependencyNode.run instanceof HelperReferences)) {
                String l = e.l(sb2, "[");
                if (dependencyNode.margin > 0) {
                    l = ba.q(e.s(l, "label=\""), "\"", dependencyNode.margin);
                    if (z) {
                        l = e.l(l, ",");
                    }
                }
                if (z) {
                    l = e.l(l, " style=dashed ");
                }
                if (dependencyNode.run instanceof HelperReferences) {
                    l = e.l(l, " style=bold,color=gray ");
                }
                sb2 = e.l(l, "]");
            }
            sb.append(sb2 + StringUtils.LF);
        }
        return sb.toString();
    }
