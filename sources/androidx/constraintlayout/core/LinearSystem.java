...
public class LinearSystem {
    ...
    private void displayRows() {
        ...
        for (int i = 0; i < this.mNumRows; i++) {
            ...
            v.append(this.mRows[i]);
            str = e.l(v.toString(), StringUtils.LF);
        }
        StringBuilder v2 = ba.v(str);
        v2.append(this.mGoal);
    ...
    public void displayReadableRows() {
        displaySolverVariables();
        String q = ba.q(new StringBuilder(" num vars "), StringUtils.LF, this.mVariablesID);
        for (int i = 0; i < this.mVariablesID + 1; i++) {
            SolverVariable solverVariable = this.mCache.mIndexedVariables[i];
            if (solverVariable != null && solverVariable.isFinalValue) {
                q = q + " $[" + i + "] => " + solverVariable + " = " + solverVariable.computedValue + StringUtils.LF;
            }
        }
        String l = e.l(q, StringUtils.LF);
        for (int i2 = 0; i2 < this.mVariablesID + 1; i2++) {
            ...
            if (solverVariable2 != null && solverVariable2.isSynonym) {
                l = l + " ~[" + i2 + "] => " + solverVariable2 + " = " + solverVariableArr[solverVariable2.synonym] + " + " + solverVariable2.synonymDelta + StringUtils.LF;
            }
        }
        String l2 = e.l(l, "\n\n #  ");
        for (int i3 = 0; i3 < this.mNumRows; i3++) {
            StringBuilder v = ba.v(l2);
            v.append(this.mRows[i3].toReadableString());
            l2 = e.l(v.toString(), "\n #  ");
        }
        if (this.mGoal != null) {
            StringBuilder s = e.s(l2, "Goal: ");
            s.append(this.mGoal);
    ...
    public void displayVariablesReadableRows() {
        ...
        for (int i = 0; i < this.mNumRows; i++) {
            if (this.mRows[i].variable.mType == SolverVariable.Type.UNRESTRICTED) {
                StringBuilder v = ba.v(str);
                v.append(this.mRows[i].toReadableString());
                str = e.l(v.toString(), StringUtils.LF);
            }
        }
        StringBuilder v2 = ba.v(str);
