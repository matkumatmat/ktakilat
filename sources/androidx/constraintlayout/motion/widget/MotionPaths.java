...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void setView(float f, View view, int[] iArr, double[] dArr, double[] dArr2, double[] dArr3, boolean z) {
        ...
        if (motionController != null) {
            ...
            if (!Float.isNaN(f15)) {
                view2.setRotation((float) (Math.toDegrees(Math.atan2((double) sin2, (double) cos)) + ((double) f15)));
            }
            f4 = f20;
            f5 = cos2;
        } else {
            ...
            f2 = f7;
            if (!Float.isNaN(f21)) {
                view2.setRotation((float) (Math.toDegrees(Math.atan2((double) ((f12 / 2.0f) + f10), (double) ((f11 / 2.0f) + f9))) + ((double) f21) + ((double) 0.0f)));
            }
        }
