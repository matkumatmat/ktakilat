...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> implements MaterialBackHandler {
    ...
    private float calculateCornerInterpolation(float f, @Nullable RoundedCorner roundedCorner) {
        if (roundedCorner != null) {
            float c = (float) roundedCorner.getRadius();
            if (c > 0.0f && f > 0.0f) {
                return c / f;
            }
        }
    ...
    private float calculateInterpolationWithCornersRemoved() {
        ...
        if (!isAtTopOfScreen() || (n = view.getRootWindowInsets()) == null) {
            return 0.0f;
        }
        return Math.max(calculateCornerInterpolation(this.materialShapeDrawable.getTopLeftCornerResolvedSize(), n.getRoundedCorner(0)), calculateCornerInterpolation(this.materialShapeDrawable.getTopRightCornerResolvedSize(), n.getRoundedCorner(1)));
    }
    ...
    private int getTopOffsetForState(int i) {
        ...
        throw new IllegalArgumentException(ba.k(i, "Invalid state to get top offset: "));
    }
    ...
    public boolean onInterceptTouchEvent(@NonNull CoordinatorLayout coordinatorLayout, @NonNull V v, @NonNull MotionEvent motionEvent) {
        ...
        if (actionMasked != 2 || view2 == null || this.ignoreEvents || this.state == 1 || coordinatorLayout.isPointInChildBounds(view2, (int) motionEvent.getX(), (int) motionEvent.getY()) || this.viewDragHelper == null || (i = this.initialY) == -1 || Math.abs(((float) i) - motionEvent.getY()) <= ((float) this.viewDragHelper.getTouchSlop())) {
            return false;
        }
        return true;
    }
