...
final class LinearDrawingDelegate extends DrawingDelegate<LinearProgressIndicatorSpec> {
    ...
    public void adjustCanvas(@NonNull Canvas canvas, @NonNull Rect rect, @FloatRange(from = 0.0d, to = 1.0d) float f) {
        ...
        canvas.translate((((float) rect.width()) / 2.0f) + ((float) rect.left), Math.max(0.0f, ((float) (rect.height() - ((LinearProgressIndicatorSpec) this.spec).trackThickness)) / 2.0f) + (((float) rect.height()) / 2.0f) + ((float) rect.top));
