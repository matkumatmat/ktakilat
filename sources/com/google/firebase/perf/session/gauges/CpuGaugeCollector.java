...
public class CpuGaugeCollector {
    ...
    private synchronized void scheduleCpuMetricCollectionOnce(Timer timer) {
        try {
            this.cpuMetricCollectorExecutor.schedule(new m4(this, timer, 1), 0, TimeUnit.MILLISECONDS);
    ...
    private synchronized void scheduleCpuMetricCollectionWithRate(long j, Timer timer) {
        ...
        try {
            this.cpuMetricCollectorJob = this.cpuMetricCollectorExecutor.scheduleAtFixedRate(new m4(this, timer, 0), 0, j, TimeUnit.MILLISECONDS);
        } catch (RejectedExecutionException e) {
            AndroidLogger androidLogger = logger;
