                        ...
                        synchronized (this.loggedChoiceIds) {
                            try {
                                if (!optString.equals(this.loggedChoiceIds.get(str))) {
                                    ...
                                    Bundle c = e.c(EXTERNAL_RC_PARAMETER_PARAM, str);
                                    c.putString(EXTERNAL_ARM_VALUE_PARAM, configs.optString(str));
                                    c.putString(EXTERNAL_PERSONALIZATION_ID_PARAM, optJSONObject.optString(PERSONALIZATION_ID));
                                    c.putInt(EXTERNAL_ARM_INDEX_PARAM, optJSONObject.optInt(ARM_INDEX, -1));
                                    c.putString("group", optJSONObject.optString("group"));
                                    analyticsConnector2.logEvent(ANALYTICS_ORIGIN_PERSONALIZATION, EXTERNAL_EVENT, c);
                                    Bundle bundle = new Bundle();
                                    bundle.putString(INTERNAL_CHOICE_ID_PARAM, optString);
                                    analyticsConnector2.logEvent(ANALYTICS_ORIGIN_PERSONALIZATION, INTERNAL_EVENT, bundle);
                                }
                            } catch (Throwable th) {
                                while (true) {
                                    throw th;
